 ...
 * save workbook
 */
function serializeWorkbook(parsedWorkbook, dest, flag) {

    for (let name in parsedWorkbook) {
        let sheet = parsedWorkbook[name];

        let dest_file = path.resolve(dest, (name + "_config" + config.json.extension).toLowerCase());
...
module.exports = {
     ...
     */
    toJson: function (src, json_dest, csharp_dest, flag) {
        ...
        serializeWorkbook(parsed_workbook, json_dest, flag);
