...
function serializeCs(dest, settings, sheetWorks, flag) {
    for (let name in settings) {
        ...
        if (setting.type == types.SheetType.MASTER) {
            ...
            strConfMgr[flag][2] += 'REPLACE_EMPTY_CODE_HERE';
            ...
            strConfMgr[flag][4] += `\t\t\t\t${name}${config.csharp.collection_suffix} = _p${name}${config.csharp.collection_suffix};\n`;
            ...
            strConfMgr[flag][4] += `\t\t\t\t${name}${config.csharp.row_suffix} = _p${name}${config.csharp.row_suffix};\n`;
...
module.exports = {
    ...
    toCsConfMgr: function (dest, flag) {
        ...
        // 使用REPLACE_EMPTY_CODE_HERE替换空代码
        strConfMgr[flag][2] = strConfMgr[flag][2].replace(/REPLACE_EMPTY_CODE_HERE/g, `\t\t\tif (IsAllLoaded) {\n${strConfMgr[flag][4]}\t\t\t}\n\n`);
