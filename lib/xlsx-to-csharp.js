...
function serializeCs(dest, settings, sheetWorks, flag) {
    ...
    for (let name in settings) {
        ...
        strConfMgr[flag][0] += "\t/// " + (setting.desc ?? '') + '\n';
        strConfMgr[flag][0] += "\t/// </summary>\n";
        if (setting.type == types.SheetType.MASTER) {
            strConfMgr[flag][0] += `\tpublic static Dictionary<${setting.id_type}, ${name}Cfg> ${name}Cfgs;\n\n`;
            strConfMgr[flag][1] += `\t\t${name}Cfgs = JsonConvert.DeserializeObject<Dictionary<${setting.id_type}, ${name}Cfg>>(loadJsonFile("${(name + (config.json.has_extension ? config.json.extension : '')).toLowerCase()}"));\n`;
        }
        else if (setting.type == types.SheetType.NORMAL) {
            strConfMgr[flag][0] += `\tpublic static ${name}Cfg[] ${name}Cfgs;\n\n`;
            strConfMgr[flag][1] += `\t\t${name}Cfgs = JsonConvert.DeserializeObject<${name}Cfg[]>(loadJsonFile("${(name + (config.json.has_extension ? config.json.extension : '')).toLowerCase()}"));\n`;

            // 如果JSON为数组且数据长度大于等于1 则生成首行数据对象
            //if (setting.dataLength >= 1) {
            strConfMgr[flag][0] += "\t/// <summary>\n";
            strConfMgr[flag][0] += "\t/// " + (setting.desc ?? '') + '(首行数据)\n';
            strConfMgr[flag][0] += "\t/// </summary>\n";
            strConfMgr[flag][0] += `\tpublic static ${name}Cfg ${name}Cfg;\n\n`;
            strConfMgr[flag][1] += `\t\t${name}Cfg = ${name}Cfgs[0];\n`;
            // }
        }
