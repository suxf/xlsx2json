                            ...
                            for (let i = 0; i < row[slave_name].length; i++) {
                                if (row[slave_name][i][head.name] != undefined && row[slave_name][i][head.name] != '') {
                                    ...
                                    try {
                                        ...
                                        let typeStrTmp = utility.detectType(typesValue[0]);
                                        if (typeStr === "") typeStr = typeStrTmp;
                                        else if (typeStr === "bool" && typeStrTmp !== "bool") typeStr = typeStrTmp;
                                        else if (typeStr === "int" && typeStrTmp === "string") typeStr = typeStrTmp;
                        ...
                        if (row[slave_name] instanceof Array) {
                            for (let i = 0; i < row[slave_name].length; i++) {
                                if (row[slave_name][i][head.name] != undefined && row[slave_name][i][head.name] != '') {
                                    ...
                                    let typeStrTmp = utility.detectType(values[0]);
                            ...
                            for (let i = 0; i < row[slave_name].length; i++) {
                                if (row[slave_name][i][head.name] != undefined && row[slave_name][i][head.name] != '') {
                                    ...
                                    try {
                                        ...
                                        // 判断typesValue数组类型
                                        if (tmp.length > 0) {
                                            let typeStrTmp = utility.detectType(tmp[0]);
                                            if (typeStr === "") typeStr = typeStrTmp;
                                            else if (typeStr === "bool" && typeStrTmp !== "bool") typeStr = typeStrTmp;
                                            else if (typeStr === "int" && typeStrTmp === "string") typeStr = typeStrTmp;
...
module.exports = {
    ...
    toCsConfMgr: function (dest, flag) {
        let strCs = "using Newtonsoft.Json;// 需安装Newtonsoft.Json开源库\n";
        strCs += "using System;\n";
