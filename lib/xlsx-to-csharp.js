...
function serializeCs(dest, settings, sheetWorks, flag) {
    ...
    for (let name in settings) {
        ...
        if (setting.type == types.SheetType.MASTER) {
            strConfMgr[flag][0] += `\tpublic static Dictionary<${setting.id_type}, ${name}Config> ${name}Configs;\n\n`;
            strConfMgr[flag][1] += `\t\t${name}Configs = JsonConvert.DeserializeObject<Dictionary<${setting.id_type}, ${name}Config>>(loadJsonFile("${(name + "_config" + config.json.extension).toLowerCase()}"));\n`;
        }
        else if(setting.type == types.SheetType.NORMAL) {
            strConfMgr[flag][0] += `\tpublic static ${name}Config[] ${name}Configs;\n\n`;
            strConfMgr[flag][1] += `\t\t${name}Configs = JsonConvert.DeserializeObject<${name}Config[]>(loadJsonFile("${(name + "_config" + config.json.extension).toLowerCase()}"));\n`;
        }

        let dest_file = path.resolve(dest, name + "Config.cs");
...
function formatCs(name, setting, sheetRows) {
    ...
    if (setting.type == types.SheetType.NORMAL) {
        strHead += indent + "public class " + name + "Config\n";
    }
