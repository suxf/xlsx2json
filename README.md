...
* 本项目是基于nodejs的，所以需要先安装nodejs环境，推荐版本：20.x。
* 获取本工具代码。
...
* `int64[]` 64位整型数组类型。
* `float[]` 单精度浮点数数组类型。
* `bool`  布尔。在确认类型的非数组情况下，只要值不为空、false、0、undefined、null、no都认为是真值。
* `bool[]`  布尔数组。
* `string` 字符串。
* `string[]` 字符串数组。
* `date` 日期类型。
...
* 首列首行（表的第一格）如果以`#`开头，那么表格式将变为垂直模式。
...
- 表明以英文符号`()`括号包裹的部分不会导出，可以作为编辑状态下提示的表名或备注。
